...
:setvar DATA_PATH "C:\\dados\\etl_lojas"
...
INSERT INTO dw.dim_transporte (nome_transportadora)
SELECT DISTINCT e.transportadora
  FROM stg.entregas e
 WHERE e.transportadora IS NOT NULL
   AND e.transportadora <> ''
   AND e.transportadora <> 'NÃO INFORMADO'
   AND NOT EXISTS (
           SELECT 1
             FROM dw.dim_transporte t
            WHERE t.nome_transportadora = e.transportadora);
GO
